...
//* the range ['x0', 'x1')                         **/
double reject(double p_max, double x0, double x1,  // p_max: max value of pdf, x0-x1 range of rand numbers. pdf: pointer to function
	      double (* pdf)(double))                 // Gen 2 random numbers x,y. If y<=pdf(x), accept x as random number. Interative process
{
...
/** artifical pdf **/
double pdf(double x) // Define artificial prob density function.
{
  if( (x<0)||
      ((x>=0.5)&&(x<1))||
      (x>1.5) )
