...
sub_array_t max_subarray(int *a, int from, int to)
{
  ...
  {
	  sa1 = max_subarray(a, from, middle);
	  sa2 = max_subarray(a, middle+1, to);
	  sa3 = max_crossing_subarray(a, from, middle, to);
  	...
  	{
	  	final.left = sa2.left;
	  	final.right = sa2.right;
	  	final.sum = sa2.sum;
  	}
  	if ((sa3.sum >= sa1.sum) && (sa3.sum >= sa2.sum))
  	{
	  	final.left = sa3.left;
	  	final.right = sa3.right;
	  	final.sum = sa3.sum;
  	}
  }
...
int main(int argc, char **argv)
{
  ...
  int lengths[12] = {10, 20, 50, 100, 200, 500, 1000, 5000, 10000, 50000, 100000, 500000};
  int l;
  ...
  for(l=0; l<12; l++)
	  {
		  ...
		  //printf("%d %f %f  %f %f \n", length, avg[1], 
			...
			// avg2[1],  sqrt( (avg2[2]-avg2[1]*avg2[1])/(stat2[0]-1) ) );
   		  printf("%d %f %f  %f %f \n", length, avg[1], 
   			 (avg[2]-avg[1]*avg[1])/(stat[0]-1),
   			 avg2[1],  (avg2[2]-avg2[1]*avg2[1])/(stat2[0]-1) );
